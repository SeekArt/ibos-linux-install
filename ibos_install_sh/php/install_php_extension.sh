...
#zend
if ls -l /ibos/server/ |grep "5.3.18" > /dev/null;then
  ...
  sed -i 's#\[opcache\]#\[opcache\]\nzend_extension=opcache.so#' /ibos/server/php/etc/php.ini
  sed -i 's#;opcache.enable=0#opcache.enable=1#' /ibos/server/php/etc/php.ini
fi